...
func (n *node) decodeObjectToMap(v reflect.Value) error {
	...
	// Reuse existing map if it exists, otherwise create new one
	var targetMap reflect.Value
	switch {
	case !v.IsNil() && v.Len() == 0:
		// Reuse empty existing map
		targetMap = v
	case !v.IsNil():
		// Clear existing map for reuse
		targetMap = v
		for _, key := range targetMap.MapKeys() {
			targetMap.SetMapIndex(key, reflect.Value{})
		}
	default:
		// Create new map only when necessary
		targetMap = reflect.MakeMap(v.Type())
		v.Set(targetMap)
	}
	...
	for key, value := range properties {
		...
		targetMap.SetMapIndex(reflect.ValueOf(key), target)
	}
